...
const SuggestionAi = ({ aiTextJson, setPreviewData }: SuggestionAiProps) => {
  ...
  const handleInsert = () => {
    dispatch(
      addSocialWatch({
        ...
        aiTypedText: true,
        previewUrl: content?.socialwatch.previewUrl,
        previewImageBool: true,
      })
    );
  };
  ...
  const handleSuggestion = async (e: any, params: any) => {
    ...
    try {
      ...
      let client_id = JSON.parse(localStorage.getItem("token") || "{}").client_id;
      const payload = {
        client_id: client_id,
      ...
      console.log(response, "response.data.captions[0]");
      ...
      dispatch(
        addSocialWatch({
          ...
          URL: content?.socialwatch.URL,
          previewUrl: content?.socialwatch.previewUrl,
          previewImageBool: content?.socialwatch.previewImageBool,
        })
      );
      if (response.data.captions[0]) {
        setProgress(false);
            ...
            <Grid container spacing={2}>
              ...
              >
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label="Word size" />
                  )}
                />
              </Grid>
            </Grid>
